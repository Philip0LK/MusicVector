 ...
 for(let line=0;line<rawRows.length;line++){
  ...
  for(const event of events){
   if(event.kind==='unknown'&&mode==='page'){
    ...
    const dots=fused?fused.dots:event.dots;
    ...
    else{byEvent.set(event.id,prior.note);if(prior.total!==null){prior.total+=24;Object.assign(prior.note.annotation,duration(prior.total));}}
