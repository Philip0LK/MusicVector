...
else if(action==='raise'||action==='lower'){if(n.degree===0)return doc;n.octave=Math.max(-1,Math.min(1,n.octave+(action==='raise'?1:-1)));const t=parse(row.text)[c.note];row.text=row.text.slice(0,t.start)+label(n)+row.text.slice(t.end)}
...
export function replaceText(doc,index,input){const text=normalizeJianpuOcrText(input).replace(/\s+/g,' ').trim(),tokens=parse(text);let rest=text;for(const t of [...tokens].reverse())rest=rest.slice(0,t.start)+rest.slice(t.end);if(!tokens.length||/[^\s|·-]/.test(rest)||tokens.some(t=>Math.abs(t.note.octave)>1))throw Error('请使用 0–7、升降号、一个高低音符号和小节线。');
