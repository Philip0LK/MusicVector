...
export function SourceRowTargets({rows,page,image,onSelect,current}) {
 const gesture=useRef(null);
 if(!image?.naturalWidth)return null;
 const regions=rows.map((row,index)=>({row,index})).filter(x=>x.row.page===page)
 ...
 return regions.map((item,i)=>{
  ...
  return <button key={item.index} className="source-row-target" data-row={item.index}
   ...
   onClick={e=>{const g=gesture.current;gesture.current=null;if(e.detail!==0&&(!g||g.moved||g.id!==item.index))return;onSelect(item.index);}}
  />;
 });
}
