...
(async()=>{
 ...
 for(let i=0;i<120;i++){
  try{const info=JSON.parse(fs.readFileSync(path.join(data,'.instance.json'),'utf8'));const r=await fetch(info.url+'/api/health',{headers:{'x-yuebeidou-token':info.token},signal:AbortSignal.timeout(500)});if(r.ok){console.log('乐北斗已开启（'+info.url+'）。关闭浏览器不会停止服务；要停止请运行 USER-GUIDE.zh-CN.md 里的停止命令。');return}}catch{}
