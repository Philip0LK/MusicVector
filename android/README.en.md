...
1. On the desktop side, open the piece and click 「发送到手机」("Send to phone") in the training area's top bar, **next to the time signature**.
...
- Confirm that the desktop side was started with `npm run dev`, and that the address shown on the panel is on the same subnet as the phone (if it was started with `npm run dev:local`, it listens only on the local machine, and the panel says so directly).
...
| When the current note has not been played yet, ←→ auditions only the current note the first time, and only changes notes the second time | 「上一音/下一音」("Previous note/Next note") follows the same rule |
...
| Automatic follow-scroll; manual scrolling stops following + 「回到当前音」("Back to current note") | Same (a finger swipe stops following) |
...
| Samples are preloaded before playback; if an individual sample fails, only that pitch is downgraded | The samples this song needs are decoded before playback; a missing pitch is simply muted |
...
What the product needs is **exact duration ratios**: an eighth note is exactly twice a sixteenth note, a dotted note exactly one and a half times that. So each note's audible window
**is exactly equal to its own duration**, and the fade in/out is scaled in proportion to the duration (fade-in ≤ 20% of the duration, tail ≤ 30%), so the fade-out does not break the ratio
...
Earlier, both sides used "duration + a fixed 35ms tail", and that fixed tail squeezed the ratio down to 1.87 (455+35 : 227+35) rather than 2;
...
├── model/      Song package parsing, playback plan, pitch (ports of the desktop JS)
...
app/src/main/assets/piano/      30 medium samples (the same naming scheme as the desktop side)
...
One round of settling on the sound quality (2026-09-18): `SoundPool` was used originally, calling `stop` at the planned moment, and since `SoundPool` has a device output latency when it starts,
...
- Receiving (with the real desktop address); the practice page's continuous "image on top, score below" scroll of the whole song; play/pause/automatic follow-scroll;
